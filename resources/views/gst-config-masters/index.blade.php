                                        ...
                                        <div id="customerList">
                                            <div class="row g-4 mb-3">
                                                <div class="col-sm-auto">
                                                    <div>
                                                        <a href="{{ route('gst-config-masters.create') }}" class="btn btn-success add-btn btn-sm">
                                            ...
                                            <table class="table align-middle table-nowrap" id="GSTTable">
...
<link rel="stylesheet" href="//cdn.datatables.net/1.13.4/css/dataTables.bootstrap5.min.css">
    ...
    @endif  
        ...
        $(function () {
            var table = $('#GSTTable').DataTable({
                ...
                paging: true,
                info: true,
                ajax: "{{ route('gst-config-masters.data') }}",
                columns: [
                    ...
                    { data: 'gst_percentage', name: 'gst_percentage', orderable: false },
                    {
                        ...
                        render: function(data, type, row) {
                            var status = data === 'approved' ? 'checked' : '';
                            return `<div class="form-check form-switch">
                                        <input class="form-check-input status-toggle" type="checkbox" data-id="${row.id}" ${status}>
                                    </div>`;
                        }
                    },
      ...
      $('#GSTTable').on('change', '.status-toggle', function () {
    var status = $(this).prop('checked') ? 'approved' : 'rejected';
    var recordId = $(this).data('id');

    $.ajax({
        url: '{{ route("gst-config-masters.update-status", ":id") }}'.replace(':id', recordId),
        method: 'POST',
        data: {
            _token: '{{ csrf_token() }}',
            status: status
  ...
  $('#deleteRecordModal').on('show.bs.modal', function(event) {
        ...
        var deleteUrl = '{{ route("gst-config-masters.destroy", ":id") }}';
