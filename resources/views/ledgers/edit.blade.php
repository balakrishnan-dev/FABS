        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="live-preview">
                    <form action="{{ route('ledgers.update' , $ledgers->id) }}" method="POST">
                        
                        @method('PUT')
                        @csrf
                        <div class="mb-3">
                            <label for="name" class="form-label">Ledger Name</label>
                            <input type="text" name="name" class="form-control" value="{{ old('name', $ledgers->name) }}">
                            @error('name')
                                <div class="text-danger mt-1">{{ $message }}</div>
                            @enderror
                        </div>
                        <button type="submit" class="btn btn-success">Update</button>
                        <a href="{{ route('ledgers.index') }}" class="btn btn-secondary">Back</a>
                    </form>
                </div>
            </div>
        </div>
